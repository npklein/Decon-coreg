...
public class NonNegativeLeastSquares
{
	...
	public double[] estimateBetaStandardErrors(int sampleSize, int numberOfTerms,
											   double[] residuals, double[] estimatedRegressionParameters,
											   double[][] observedValues){
		// based on http://faculty.cas.usf.edu/mbrannick/regression/Part3/Reg2.html

		// first calculate numerator, s^2 (see above link)
		//int degreesOfFreedom = (sampleSize - numberOfTerms) - 1;
		double[] standardErrors = new double[predictedValues.length];
		//double sumOfSquareError = new Double(0);
		//for(double error : residuals){
		//	sumOfSquareError += error*error;
		//}
		//double S = new Double(sumOfSquareError/degreesOfFreedom);


		// then calculate denominator, (sum of squares B1)*(1-r^2)   -> do same for B2, B3 etc
		// get the mean of X1, X2, X3, etc
		double [] mean_per_term = new double[numberOfTerms];
		for (double[] row : observedValues)
		{
			if(numberOfTerms != row.length) {
				System.out.println(numberOfTerms);
				System.out.println(row.length);
				throw new RuntimeException("number of terms and row length observed values should be the same");
			}
			for (int i = 0; i < row.length; i++)
			{
				mean_per_term[i] += row[i];
			}
		}
		// calculate sum of squares of X1, X2, X3, etc
		double[] sumOfSquares = new double[numberOfTerms];
		Arrays.fill(sumOfSquares, 0);
		for (double[] row : observedValues)
		{
			if(numberOfTerms != row.length) {
				System.out.println(numberOfTerms);
			...
			for (int i = 0; i < row.length; i++)
			{
				sumOfSquares[i] += (row[i]-mean_per_term[i])*(row[i]-mean_per_term[i]);
			}
		}
		// calcul
